...
async function requestAssistantReply(messages: ChatMessage[]) {
  ...
  if (!apiKey) {
    return {
      reply:
        'برای دریافت پاسخ‌های زنده از دستیار هوشمند مانی اویل لازم است متغیر ASM_ASSISTANT_API_KEY در محیط سرور تنظیم شود. در حالت نمایشی فعلی، این پاسخ به صورت داخلی تولید شده است: برای انتخاب روغن مناسب، دفترچه خودرو، شرایط آب و هوا و نحوه عملکرد موتور را در نظر بگیرید و از ویسکوزیته پیشنهادی سازنده خارج نشوید.',
      fallback: true
    } as const;
  }
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({
      model: MODEL_NAME,
      systemInstruction:
        'شما دستیار هوشمند مانی اویل هستید. همیشه به فارسی پاسخ دهید، روی انتخاب روغن، فیلتر و نگهداری خودرو تمرکز کنید، و در صورت نیاز مشتری را به کارشناسان انسانی ارجاع دهید. از ارائه مشاوره‌های خطرناک خودداری کنید و اگر اطلاعات کافی در اختیار ندارید صادقانه اعلام نمایید.'
    });
    ...
    return {
      reply:
        'در حال حاضر اتصال به دستیار هوشمند مانی اویل با مشکل مواجه شده است. لطفاً بعداً مجدداً تلاش کنید یا از طریق پشتیبانی انسانی سوال خود را مطرح نمایید.',
      fallback: true
    } as const;
  }
}
